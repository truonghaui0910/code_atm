...
class ProcessUtils {
    ...
    //$processName tên process, $unlock: nếu vẫn bị khóa sau $unlock phút thì tự động mở khóa. $unlock = 0 => ko bật chế độ tự mở khóa
    public static function isfreeProcess($processName, $unlock = 0) {
        ...
        if ($lockProcess) {
            if ($lockProcess->status == 0) {
                ...
                if ($unlock == 0) {
                    ...
                    if (time() - $lockProcess->time >= $unlock * 60) {
                        return true;
                    } else {
                        return false;
                    }
